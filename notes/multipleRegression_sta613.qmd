...
- Test for the regression 
  ...
  $$
  F = \frac{SS_{reg}/p}{MSE}
  $$
  This has an F-distribution with $(p, n-p)$ degrees of freedom.
...
$$
\begin{aligned}
y_{p,d,s,g} &= \beta_{d,s} + \gamma_p + \epsilon_{p, d, g, s}\\
\epsilon_{p,d,s,g} &\sim N(0, \sigma^2)
\end{aligned}
$$
...
$$
\begin{aligned}
y_{p,d,s,g} &= \beta_{d,s} + \delta_{d,s} I_{geno='Tg'} + \gamma_p + \epsilon_{p, d, g, s}\\
\epsilon_{p,d,s,g} &\sim N(0, \sigma^2)
\end{aligned}
$$
...
anova(lm.null,lm.out1)
```

Large F-statistics yields a small p-value, which implies evidence against the null. Genotype appears to be an important predictor.
...
$$
\begin{aligned}
y_{p,d,s,g} &= \alpha_s + \beta_{s}d + \delta_sI_{geno = 'Tg'} + \mu_s I_{geno='Tg'}d + \gamma_p + \epsilon_{p, d, g, s}\\
\epsilon_{p,d,s,g} &\sim N(0, \sigma^2)
\end{aligned}
$$
...
lm.out3<-lm(y ~ 
              ...
              isTg:substrate + isTg:doseX:substrate + pair,data=o2)
```
## Summary
...
$$
\begin{aligned}
y_{p,d,s,g} &= \alpha_s + \beta_{s}\text{poly}(d,2) + \delta_sI_{geno = 'Tg'} + \mu_s I_{geno='Tg'}\text{poly}(d,2) + \gamma_p + \epsilon_{p, d, g, s}\\
\epsilon_{p,d,s,g} &\sim N(0, \sigma^2)
\end{aligned}
$$
